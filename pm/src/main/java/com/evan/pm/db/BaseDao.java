...
public abstract class BaseDao<T, ID> {
    ...
    public int save(T t) {
        ...
        try {
            ...
            rollBack(dao, databaseConnection);
        } finally {
            endConn(dao, databaseConnection);
        }
        return 0;
    }
     ...
     * @throws SQLException SQLException异常
     */
    public List<T> query(String columnName, String columnValue) {
        Dao<T, ID> dao = getDao();
        DatabaseConnection databaseConnection = null;
        try {
    ...
    private void rollBack(Dao dao, DatabaseConnection conn) {
        try {
            if (dao != null && conn != null) {
                dao.rollBack(conn);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private void endConn(Dao dao, DatabaseConnection conn) {
        try {
            if (dao != null && conn != null) {
